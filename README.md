...
| 平台 | 监听源 | 实时路径 |
| --- | --- | --- |
| Windows | WASAPI 回环捕获 | 捕获线程 → 严格 SPSC RingBuffer → 自适应读取器 → WASAPI 共享模式渲染线程 |
...
队列物理容量为 50 ms，但它不是固定监听延迟。正常水位、WASAPI 缓冲、设备驱动和硬件都会共同影响实际端到端延迟。
